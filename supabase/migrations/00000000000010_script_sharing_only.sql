-- ================================================================
-- Add share_key to scripts + create script_shares table
-- (scripts table already exists)
-- ================================================================

-- 1. Add share_key column
...
-- RLS policies for script_shares
DROP POLICY IF EXISTS "script_shares: owner read"           ON public.script_shares;
DROP POLICY IF EXISTS "script_shares: owner delete"         ON public.script_shares;
DROP POLICY IF EXISTS "script_shares: authenticated insert" ON public.script_shares;
DROP POLICY IF EXISTS "script_shares: own read"             ON public.script_shares;

CREATE POLICY "script_shares: owner read"
  ON public.script_shares FOR SELECT
  USING (script_id IN (SELECT id FROM public.scripts WHERE owner_id = auth.uid()));
...
-- 3. Allow shared users to read scripts
DROP POLICY IF EXISTS "scripts: shared users read" ON public.scripts;
CREATE POLICY "scripts: shared users read"
  ON public.scripts FOR SELECT
  USING (id IN (SELECT script_id FROM public.script_shares WHERE user_id = auth.uid()));

-- 4. Allow shared users to read dialogs
...
-- 5. Function to accept a share key
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    WHERE pubname = 'supabase_realtime' AND tablename = 'script_shares'
  ) THEN
